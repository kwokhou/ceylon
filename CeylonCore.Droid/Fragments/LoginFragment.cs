...
namespace CeylonCore
{
	public class LoginFragment : Fragment
	{
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="Ceylon.iOS.LoginViewController"/> class.
		/// </summary>
		...
		{
			if (string.IsNullOrEmpty (CeylonAccount))
		...
		View CreateInstructions (LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState)
		{
			...
			var coloredText = Html.FromHtml ("<font color='#48D1CC'></font>;");
		...
		async void LoadUserImage()
		{
			...
			var data = await Gravatar.GetImageBytes (CeylonAccount, px);
			var image = await BitmapFactory.DecodeByteArrayAsync (data, 0, data.Length);
			imageView.SetImageDrawable (new CircleDrawable (image));
		}

		// Andy: This will be EnteredbyUser
		readonly string CeylonAccount = "andy.yong@userdomain.com";
		async void Login (string username, string password)
		{
			...
			if (success) {
				...
				Toast.MakeText (this.Activity, "Please verify your Ceylon account credentials and try again", ToastLength.Long).Show();
