...
namespace CeylonCore
{
	public class WebService
	{
		...
		public async Task<List<Product>> GetTags()
		{
			if (products == null) {
				products = await Task.Factory.StartNew (() => {
					try {
						var request = CreateRequestX("http://www.mocky.io/v2/53d127f08d3e52480f7fb92b");
		...
		public async Task PreloadImages(float imageWidth)
		{
			...
			await Task.Factory.StartNew (() => {
				...
				imagUrls.ForEach( async (x) => await XFileCache.Download(x));
		...
		public Task<List<Country>> GetCountries()
		{
			return Task.Factory.StartNew (() => {
				try {
					...
					var request = CreateRequestX ("http://www.mocky.io/v2/53d12bab8d3e52790f7fb92d");
					string response = ReadResponseText (request);
		...
		static HttpWebRequest CreateRequestX(string url)
		{
